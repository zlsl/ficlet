...
public class HypenateManager {
    ...
    public static boolean canHypenate(FixedCharSequence word, float maxWidth, float[] widths, float dashWidth  /*FontStyle paint*/, Object[] hypened) {
        ...
        for (int i = len; i >= 0; i--) {
            for (HypenatePattern s_pattern : s_patterns)
                if (len - i >= s_pattern.PatternLength && len - i - s_pattern.Position > 2 && i + s_pattern.Position > 2) {

                    //Log.d("HypenateManager", "Comparing '" + test + "' to pattern '" + s_patterns[j].Pattern + "'");

                    if (buffer.equalsPart(i, i + s_pattern.PatternLength, s_pattern.Pattern, s_pattern.PatternLength)) {
                        found = true;
                        length = i + s_pattern.Position;
                        break;
                    }
                }
            if (found)
